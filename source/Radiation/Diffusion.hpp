...
*/
    Diffusion(DiffusionCoefficientCalculator const& D_coefficient_calc, DiffusionBoundaryCalculator const& boundary_calc,
        EquationOfState const& eos, std::vector<std::string> const zero_cells = std::vector<std::string> (), bool const flux_limiter = true, bool const hydro_on = true) : D_coefficient_calcualtor(D_coefficient_calc),
        boundary_calc_(boundary_calc), eos_(eos), flux_limiter_(flux_limiter), hydro_on_(hydro_on),sigma_planck(), fleck_factor(), CG::MatrixBuilder(zero_cells) {}
    ...
    mutable std::vector<double> sigma_planck, fleck_factor, D,R2, cell_flux_limiter;
